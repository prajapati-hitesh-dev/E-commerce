...
function initSpinWheel() {
    ...
    wheel.innerHTML = prizes.map((prize, index) => `
        <div class="absolute w-full h-full ${prize.color} flex items-center justify-center text-white font-bold text-sm" 
             style="transform: rotate(${index * 45}deg); clip-path: polygon(50% 50%, 0% 0%, 29.3% 29.3%); color: white; text-shadow: 0 1px 2px rgba(0,0,0,0.6)">
            <span style="display:inline-block; transform: rotate(-22.5deg);">${prize.label}</span>
        </div>
    `).join('');
}
